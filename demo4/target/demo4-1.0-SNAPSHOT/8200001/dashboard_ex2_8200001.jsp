	...
	%>
		...
		<div class="container theme-showcase" role="main">
			...
			<%
				...
				List<User> users = usr_dao.getUsers();
			...
			<%
						for (int i = 0; i < users.size(); i++){
							request.setAttribute("uname", users.get(i).getUsername());
							if(user.getUsername().equals(users.get(i).getUsername())){
			%>
								<tr class="success">
									<th class="success"><%=i+1%></th>
									<td class="success"><%=users.get(i).getLastname()%></td>
									<td class="success"><%=users.get(i).getFirstname()%></td>
									<td class="success">
										<a href="profile_ex3_8200001.jsp?uname=<%=users.get(i).getUsername()%>"><%=users.get(i).getEmail()%></a>
									</td>
								</tr>
			<%
							}else {
			%>
								<tr>
									...
									<td><a href="profile_ex3_8200001.jsp?uname=<%=users.get(i).getUsername()%>"><%=users.get(i).getEmail()%></a></td>
								</tr>
			<%
						}
				}
